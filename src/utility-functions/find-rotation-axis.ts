...
export function findRotationAxis( AnimationData: [][] ): [string, boolean] {
    let axis = ''
    ...
    if( rotationsEqual ){
        ...
        // back to zero
        const axisAsIndex = AnimationData[ 3 ].findIndex( ( element: number) => element !== 0 );
        if (axisAsIndex === 0 ) axis = 'x'
        else if (axisAsIndex === 1 ) axis = 'y'
        else if (axisAsIndex === 2 ) axis = 'z'
        else axis = 'x'; //default if all the rotations are zero, we will rotate zero on x-axis
    } else {
        const deltas = subtractEachIndexOf2Arrays( AnimationData[ 2 ], AnimationData[ 3 ] );
