...
export default function HomePage(props) 
{
  ...
  return (
    <>
        ...
        { !props.cameraRotate ? <HeroOverlay /> : ''}
...
function RotateCamera({cameraRotate}) {
  useFrame((state) => 
  {
    ...
    state.camera.rotation.y = THREE.MathUtils.lerp(state.camera.rotation.y, (cameraRotate ? (Math.PI) : 0), 0.1)
  })
  return <></>
}
...
function TestosteroneModel(props) {
    ...
    useFrame((state) => {
        ...
        ref.current.rotation.x = THREE.MathUtils.lerp(ref.current.rotation.x, props.flipped ? (Math.PI * 1.5) : (Math.PI / 2) , 0.2)
    })
...
function LessonSelectionOverlay(props) {
  function Card(props) 
  {
    ...
    return (
      // <div className="frame" onMouseMove={mouseMove} onMouseLeave={mouseLeave} ref={frame} onClick={() => props.setPage(`${props.id}_Lesson`)}>
